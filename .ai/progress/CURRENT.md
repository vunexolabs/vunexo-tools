...
- Nothing is currently uncommitted-and-forgotten: this session's tax work, audit fixes, and UI redesign are all still sitting in the working tree, not yet committed. Whoever picks this up next should check `git status` and decide whether to commit before doing anything else.
