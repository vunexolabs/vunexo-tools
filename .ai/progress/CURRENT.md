...
- **V2 Rounds 1–6 locked — all design rounds done** (2026-08-30 session 12) — `.ai/product-v2.md`, `docs/vunexo-billing/user-flows-v2.md`, `docs/vunexo-billing/database-schema-v2.md`, `docs/vunexo-billing/application-architecture-v2.md`, `docs/vunexo-billing/ui-ux-v2.md`, `docs/vunexo-billing/calculation-engine-v2.md`. Second tax regime is `VAT_STANDARD` (deliberately narrow — flat per-line rate, no jurisdiction/exemption logic), found to need **zero changes** to `calculate_invoice`'s core algorithm, only a `present_vat` presentation function. Quote state machine (incl. `ACCEPTED → CANCELLED`), own numbering series, tax-regime-as-business-setting with a `tax_regime_snapshot` column enforcing no-retroactive-recalc, statements/reports as pure SQL read models, no send-tracking for reminders, `ConvertQuoteToInvoice`'s atomic 2-table transaction, Quotes/Reports as new sidebar sections with Statement/Reminder kept as customer-tab/modal rather than new top-level sections. **Every design round (1–6) is now locked. Nothing started on Round 7 (implementation) yet** — that's the next and only remaining step before V2 has real code.
