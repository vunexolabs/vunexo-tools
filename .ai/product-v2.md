...
6. Calculation engine changes — **done** (`docs/vunexo-billing/calculation-engine-v2.md`): second regime named (`VAT_STANDARD`, deliberately narrow — flat per-line rate, no jurisdiction/exemption/reverse-charge logic), found to need zero changes to `calculate_invoice`'s core steps (verified by hand), only its own presentation function (`present_vat`, symmetric with `split_gst`). Two golden test vectors added; `database-schema-v2.md`/`application-architecture-v2.md`'s deferred enum/CHECK widenings filled in as amendments.
7. Implementation
8. Testing
9. Release
