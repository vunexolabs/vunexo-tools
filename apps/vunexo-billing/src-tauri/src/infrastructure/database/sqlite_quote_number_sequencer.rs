//! Mirrors `sqlite_invoice_number_sequencer.rs` exactly, against
//! `quote_number_counters` instead of `invoice_number_counters` —
//! database-schema-v2.md §6.
...
pub struct SqliteQuoteNumberSequencer {
    pool: SqlitePool,
}

impl SqliteQuoteNumberSequencer {
...
fn format_quote_number(format: &str, year: i32, seq: i64) -> String {
...
#[async_trait]
impl QuoteNumberSequencer for SqliteQuoteNumberSequencer {
    async fn preview_next(
        ...
        let last_value: Option<i64> =
            sqlx::query_scalar("SELECT last_value FROM quote_number_counters WHERE scope_key = ?")
                .bind(&scope_key)
                .fetch_optional(&self.pool)
                .await?;
        Ok(format_quote_number(
            format,
            at.year(),
    ...
    async fn issue_next(
        ...
        sqlx::query(
            "INSERT INTO quote_number_counters (scope_key, last_value) VALUES (?, 1) \
        ...
        let new_value: i64 =
            sqlx::query_scalar("SELECT last_value FROM quote_number_counters WHERE scope_key = ?")
                ...
                .fetch_one(&mut **conn)
                .await?;
        Ok(format_quote_number(format, at.year(), new_value))
    }
}
...
mod tests {
    ...
    fn formats_default_style() {
        assert_eq!(
            format_quote_number("QUO-{year}-{seq:04d}", 2027, 12),
            "QUO-2027-0012"
        );
    }
    ...
    fn scope_key_resets_yearly_only_for_year_based_formats() {
        let date = NaiveDate::from_ymd_opt(2027, 1, 5).unwrap();
        assert_eq!(scope_key_for("QUO-{year}-{seq:04d}", date), "2027");
        assert_eq!(scope_key_for("QUOTE-{seq}", date), "ALL");
    }
}
